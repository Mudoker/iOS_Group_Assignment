        ...
        GeometryReader { proxy in
            VStack {
                ...
                ZStack (alignment: .topLeading) {
                    ...
                    TextField("", text: $homeVM.editPostCaption, prompt:  Text("Share your updates...").foregroundColor(isDarkModeEnabled ? .white.opacity(0.5) : .black.opacity(0.5))
                        .font(.title3)
                    )
                ...
                Button(action: {
                    Task {
                        let _ = try await homeVM.editCurrentPost(postID: post.id, newPostCaption: homeVM.editPostCaption, newMediaURL: homeVM.newPostSelectedMedia, editSelectedTag : homeVM.editSelectedPostTag)
...
//                homeVM.selectedUserTag.removeAll()
            ...
            .sheet(isPresented: $isOpenUserListViewOnIpad) {
                ...
                PostTagListView(proxy: $proxySize, isDarkModeEnabled: isDarkModeEnabled, searchTagText: $homeVM.postTagListSearchText, selectedTags: $homeVM.editSelectedPostTag, isShowPostTagList: $isOpenPostTagListViewOnIphone, filteredTags: filteredTags)
            }
            .sheet(isPresented: $isOpenPostTagListViewOnIpad) {
                PostTagListView(proxy: $proxySize, isDarkModeEnabled: isDarkModeEnabled, searchTagText: $homeVM.userTagListSearchText, selectedTags: $homeVM.editSelectedPostTag, isShowPostTagList: $isOpenPostTagListViewOnIpad, filteredTags: filteredTags)
            }
        }
